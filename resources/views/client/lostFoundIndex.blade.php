...
<body>
	...
	<div class="nav col-md-10 col-md-offset-1 col-sm-12 col-xs-12">
		...
		<div class="nav-wrap col-md-12 col-sm-12 col-xs-12" style="padding: 0;">
			<ul class="top-navgation1 col-md-6 col-sm-12 col-xs-12 pull-right">
				<li><a href="{{url('home/noticeIndex')}}">
					首页
				</a></li>
				<li><a href="{{url('home/typeIndex')}}/寻物启事">
					寻物
				</a></li>
				<li><a href="{{url('home/typeIndex')}}/招领启事">
					招领
				</a></li>
				<li><a href="{{url('home/noticeUserCenter')}}">
					个人中心
				</a></li>
				<li><a data-toggle="modal" data-target="#myModal" class="publish" style="color:#fff; cursor:pointer;">发布+</a></li>
			</ul>
		</div>
		<!--end wrap-->
	...
	<div class="content col-md-10 col-md-offset-1 col-sm-12 col-xs-12" style="padding: 0;">
		<div class="col-md-6 col-xs-12" style="padding: 0;">
			<img src="{{asset('image')}}/thank.jpg" />
		</div>
		<div class="col-md-6 col-xs-12" style="padding: 0;">
			<h4>最新动态：</h4>
			<ul>
				<li>1、共<span>{{$countLost}}</span>条寻物启事</li>
				<li>2、共<span>{{$countFound}}</span>条招领启事</li>
				<li>3、共有<span>{{$countComment}}</span>条评论</li>
				<li>4、<span>注:骗子经常利用本站信息进行诈骗，谨防上当受骗！</span></li>
			</ul>
		</div>
	</div>
	<!--end content-->

	<div class="mid col-md-10 col-md-offset-1 col-sm-12 col-xs-12">
		<h4>最新失物招领启事</h4>
	</div>
	<!--end mid-->
	<div class="list col-md-10 col-md-offset-1  col-sm-12 col-xs-12">


		@foreach($data as $v)

		<div class="grid col-md-3 col-sm-4 col-xs-12">
			<div class="grip-wrap">
				<a href="{{url('home/detail')}}/{{$v->id}}">
					@if($v->picture=='')
					<div style="background: #ccc;height: 100px;font-size: 24px;color: #fff;line-height: 100px;text-align: center;">暂无相关图片</div>
					@else
					<img src="{{asset('')}}{{$v->picture}}" class="img-responsive" />
					@endif
					<span>{{$v->title}}</span>
					<hr />
					<h5>{{$v->type}}</h5>
					<p>{{$v->description}}</p>
					<p style="padding: 0;text-align: right;">发布日期：<br>{{$v->date}}</p>
